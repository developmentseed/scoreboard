      ...
      return ({ cell: { value } }) => {
        ...
        return (
          <Link href={`/country?code=${code}`} as={`/countries/${code}`}>
            <a className='link--normal'>{value}</a>
          </Link>
        )
      }
    case 'campaignlink':
...
function prepareAllHeaders (table) {
  ...
  headers.sort(
    (a, b) => table.columnOrder.indexOf(a.id) - table.columnOrder.indexOf(b.id)
  )
  ...
  headers.forEach(
    (header) =>
      (headerObjects[header.id] = header.displayTooltip ? (
        <Tooltip
          dataTip={header.description}
          className={
            table.headers[header.id].type === 'number'
              ? 'table-align-right'
              : ''
          }
          style={{
            justifyContent:
              table.headers[header.id].type === 'number' ? 'flex-end' : ''
          }}
        >
          {header.name}
        </Tooltip>
  ...
  return (
    <table>
          ...
          {headers.map((column) =>
            column.canSort ? (
              <th
                ...
                title={
                  column.Header.props
                    ? `Sort by ${column.Header.props.children}`
                ...
                <a
                  className={
                    (column.isSorted
                      ...
                      : 'sort-none') +
                    ' ' +
                    (column.Cell === formattedNum || column.Cell === timeString
                      ? 'table-align-right'
                      : '')
                  }
                >
              ...
              {row.cells.map((cell) => {
                return (
                  <td
                    {...cell.getCellProps()}
                    className={
                      (!isNaN(cell.value) && parseInt(cell.value) >= 0) ||
                      cell.column.Cell === timeString
                        ? 'table-align-right'
                        : ''
                    }
                  >
      ...
      <tfoot>
        <tr>
          {headers.map((column) => (
            <td key={column.id} className={'table-align-right'}>
              {column.Footer}
            </td>
          ))}
        </tr>
