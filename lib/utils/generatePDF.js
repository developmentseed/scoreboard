...
function drawPageBorders (doc, options) {
  ...
  doc.rect(options.pageWidth - 50, 0, 50, 50).fill('#ffffff')
  doc.rect(0, options.pageHeight - 50, 50, 50).fill('#ffffff')
  doc.rect(options.pageWidth - 50, options.pageHeight - 50, 50, 50).fill('#ffffff')
}

// be sure to replace all "newBadges" with "badges"
function printBadges (doc, options, newBadges) {
  // let badgeCount = Object.keys(newBadges).length
  // let pageCount = 1
  // if (badgeCount > 10) { pageCount++ }
  // if (pageCount === 2) {
  //   drawPageBorders(doc, options)
  // }
  return Promise.all(Object.keys(newBadges).map((x, i) => {
    let earnedBadge = newBadges[x]
    ...
    console.log('i is: ' + i + ' and rowCount is: ' + rowCount)
    console.log('yPos is ' + yPos + ' and xPos is ' + xPos)
...
export default async function generatePDF (name, earnedBadges) {
  ...
  await printBadges(doc, options, newBadges)
  doc.end()
