...
export default async function generatePDF (name, earnedBadges) {
  ...
  let subTitleFont = await fetch('static/fonts/Abel-Regular.ttf')
    .then(response => response.arrayBuffer())

  let baseFont = await fetch('static/fonts/Heebo-Bold.ttf')
    .then(response => response.arrayBuffer())

  let Logo = await fetch('static/android-chrome-192x192.png')
    .then(response => response.arrayBuffer())
