...
/**
 * Check if osm id is in set of moderator ids for a team.
 *
 * @param {number} osmId
 * @param team
 * @returns {boolean}
 */
function isModerator (osmId, team) {
  if (!osmId) return false
  const mods = new Set(team.moderators.map(({ osm_id }) => osm_id))
  return mods.has(osmId)
}
