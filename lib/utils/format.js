import { distanceInWords, subDays } from 'date-fns'
...
export function formatTimeDescription (timeVar) {
  const now = new Date()
  // divide difference in timestamps by millisecond conversion
  const daysDifference = (now - timeVar) / (1000 * 60 * 60 * 24)
  if (daysDifference < 1) {
    return 'today'
  } else return `${distanceInWords(new Date(), timeVar)} ago`
}
