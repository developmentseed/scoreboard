...
function getUserInfo () {
  return fetch(join(APP_URL_PREFIX, '/auth/userinfo'))
    .then(res => {
      if (res.status === 200) {
        return res.json()
      } else {
...
const userActions = store => ({
  ...
  **/
  getAuthenticatedUser (id) {
    return getUserInfo()
      .then(osm => {
        if (!osm) return
        return fetch(join(APP_URL_PREFIX, '/api/users/', osm.id))
          .then(res => {
            if (res.status === 200) {
              return res.json()
            }
  ...
  getProjects () {
    return fetch(join(APP_URL_PREFIX, '/api/projects/'))
      .then(res => {
        if (res.status === 200) {
          return res.json()
        } else {
  ...
  getRoles () {
    return fetch(join(APP_URL_PREFIX, '/api/roles'), { credentials: 'include' })
      .then(res => {
        if (res.status === 200) {
          return res.json()
        } else {
  ...
  **/
  adminGetUser (state, id) {
    return fetch(join(APP_URL_PREFIX, '/api/users/', id), { credentials: 'include' })
      .then(res => {
        if (res.status === 200) {
          return res.json()
        } else {
  ...
  **/
  adminGetUsers () {
    return fetch(join(APP_URL_PREFIX, '/api/users'), { credentials: 'include' })
      .then(res => {
        if (res.status === 200) {
          return res.json()
        } else {
  ...
  updateUserRoles (state, userId, roles) {
    return fetch(join(APP_URL_PREFIX, '/api/users/', userId), {
      method: 'PUT',
      credentials: 'include',
  ...
  createBadge (state, params) {
    return fetch(join(APP_URL_PREFIX, '/api/badges'), {
      method: 'POST',
      credentials: 'include',
  ...
  async getTopStats () {
    return fetch(join(APP_URL_PREFIX, '/api/topstats'))
      .then(async (res) => {
