...
export default store => ({
  ...
  createTasker (state, params) {
    return fetch('/api/taskers', {
      ...
      store.setState({ notification: { type: 'success', message: 'Tasker created successfully' } })
    })
  },
  ...
  updateTasker (state, id, data) {
    return fetch(`/api/taskers/${id}`, {
      ...
      store.setState({ notification: { type: 'success', message: 'Tasker updated successfully' } })
    })
  },
    ...
    fetch(`/api/taskers/${id}`)
      .then(res => {
        if (res.status === 200) {
          ...
          throw new Error('failed to get Tasker')
        }
      })
      .then((tasker) => {
  ...
  deleteTasker (state, id) {
    return fetch(`/api/taskers/${id}`, {
      ...
      store.setState({ notification: { type: 'success', message: 'Tasker deleted successfully' } })
    }).catch((error) => {
      store.setState({ notification: { type: 'error', message: error.message } })
    })
  }
